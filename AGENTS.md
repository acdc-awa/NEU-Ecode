...
- `SettingsActivity` — account button renders 登录/切换账号 by current session state; QR brightness mode dropdown (FULL default / HDR experimental / SYSTEM); 开屏二维码显示 dropdown (`qrShowOnLaunch`); back-press mode radio buttons; the 关于 section hosts the online-update state machine (see below)
...
- Off-campus logins hit a second factor: the credential POST alone yields the login page again, and completing it needs an image captcha (`GET /tpass/code` → `imgCode`, sent to `/tpass/secondAuthCode?code=…&method=mobile` to trigger the SMS) **plus** the SMS code (`scendAuthCode`) in the same final form POST (`imgCode=…&scendAuthCode=…&method=mobile&RelayState=…&execution=e2&_eventId=submit`). No trusted-device token exists, so replaying stored credentials cannot produce a silent off-campus login — see `.har/webvpn.neu.edu.cn.har`
- Portal balance consumes `SESS_ID` + `CK_LC` + `CK_VL` (all `personal.neu.edu.cn`). `CK_LC`/`CK_VL` are minted when `cas_login/1` consumes the ticket; `SESS_ID` is minted by `/portal/` and is a session cookie until some portal endpoints re-issue it with `Max-Age=86400` (24 h). The balance endpoints (`personal/frontend/data/*`) do **not** refresh it — only e.g. `/portal/ucs/frontend/msg/index` does
- Therefore the fix for the "balance dies after a while, only re-login recovers" bug is **not** to refresh CASTGC (impossible off-campus) but to keep the portal session sliding: `PortalClient.keepSessionAlive()` pings the `msg/index` URL, called on every `MainActivity.onResume` and every 30 min while foregrounded (`PORTAL_KEEPALIVE_INTERVAL_MS`). As long as the app is opened within any 24 h window the portal session never lapses
...
2. `CasSessionRenewer.renewEcodeSession()` — silent renewal: GETs the same CAS login URL; a valid `CASTGC` gets 302'd straight through with a ticket to ecode's sso/login, which re-plants `XSRF-TOKEN`. Success = landed host is ecode + `hasEcodeSession()`; false means CASTGC expired; IOException propagates (callers classify it as `NetworkError`). Only works within 2 h of the last real login — see "CAS/CASTGC session lifetime"
