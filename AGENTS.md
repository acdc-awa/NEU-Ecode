...
- Balance: `PortalClient.fetchBalance()` returns a sealed `BalanceResult` (`Success`/`NetworkUnreachable`/`Failed`). Balance loads once automatically per session (on login return) and afterwards only manually
